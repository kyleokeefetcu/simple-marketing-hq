    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto w-full max-w-6xl px-5 py-8">
        ...
        <div className="mt-5 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-3 text-4xl font-semibold text-slate-950">Generate clean marketing assets from saved business context.</h1>
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            Each lab tool uses the selected business, diagnostic answers, website analysis, and saved assets to return concise copy, offers, insights, and next actions.
          </p>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {marketingLabPromptPacks.map((prompt) => (
            <Link key={prompt.role_id} href={rolePath(prompt.role_id)} className="group rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-md">
              <p className="text-xs font-semibold uppercase tracking-wide text-cyan-800">Marketing asset</p>
